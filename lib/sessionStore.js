...
// Vercel and other serverless targets mount the application directory as read-only.
// When we detect such an environment and the session file path hasn't been overridden,
// default directly to the writable /tmp directory instead of failing the first write.
const prefersFallback =
  !CUSTOM_SESSION_FILE && (process.env.VERCEL || process.env.AWS_REGION || process.env.NOW_REGION);
const DEFAULT_SESSION_FILE = CUSTOM_SESSION_FILE
  ? CUSTOM_SESSION_FILE
  : prefersFallback
    ? FALLBACK_SESSION_FILE
    : path.join(process.cwd(), 'data', 'sessions.json');
const CAN_FALLBACK = !CUSTOM_SESSION_FILE && !prefersFallback;
